...
{{--                            <span class="label label-{{ ($dataInfos[0]->status == 0) ? 'warning': 'success' }}">{{ ($dataInfos[0]->status == 0) ? 'Unprocessed': 'Processed' }}</span>--}}
                        ...
                        @if(Auth::user()->hasRole('manager') and (\App\Http\Controllers\DataController::checkStatus($dataInfos[0]->uniqueID)) == 0)
                            <a href="#" class="btn btn-success btn-sm pull-right custom-btn" data-toggle="modal" data-target=".approval-modal"><i class="fa fa-check"></i> Approve Now</a>
                            <a href="{{ url('data/edit/'. $dataInfos[0]->uniqueID) }}" class="btn btn-warning btn-sm pull-right custom-btn"><i class="fa fa-edit"></i>Edit & Approve</a>
...
{{--                                    <td><b><a href="#">{{ $dataInfo->filename }}</a></b></td>--}}
                                    ...
                                    <td><a class="custom-url map-marker" target="_blank" href="http://maps.google.com/maps?q={{ $dataInfo->lat1 }},{{ $dataInfo->lon1 }}" data-toggle="modal" data-target=".{{ substr($dataInfo->filename, 16, 6) }}" data-placement="top" title="Latitude and Longitude {{ $dataInfo->lat1 . ', '. $dataInfo->lon1 }}"><i class="fa fa-map-marker"></i></a></td>
                                    <!-- Large modal -->
                                    ...
                                    <!-- modal for google map -->
                                    <td><a class="custom-url icon-download" target="_blank" href="download/{{ $dataInfo->filename  }}" data-toggle="tooltip" data-placement="top" title="Download File : {{ $dataInfo->filename }}"><i class="fa fa-download"></i></a></td>
                                ...
                                <div class="box-body">
                                    <table id="emailTable" class="table table-bordered table-striped">
                                        ...
                                        <tr>
                                            ...
                                            <td> {{ $summaries->latitude }}, {{ $summaries->longitude }} <a  href="#"  data-toggle="modal" data-target=".{{ $summaries->operationID }}" class="map-marker" data-toggle="tooltip" data-placement="top" title="Latitude and Longitude"><i class="fa fa-map-marker"></i></a></td>
                                            <!-- google map -->
                                            <!-- modal for google map -->
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box">
                    ...
                    <div class="box-body">
                        <div class="row">
                            <div class="col-md-{{ (!$summaries) ? '10' : '12' }}">
                            ...
                            <!-- /.col -->
							<!-- status 0 == unprocessed and status == 5 is flagged -->
                            @if($dataInfos[0]->status == 0 or $dataInfos[0]->status == 5)
                                ...
                                <form role="form" method="POST" action="{{ route('summery.store') }}">
                                    @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control delectLevel" id="top_snow" name="top_snow" placeholder="Top of Snow">
                                    </div>
                                    <!-- snow level -->
                                    <div class="form-group">
                                        <label for="top_ice">Top of Ice</label>
                                        <input type="text" class="form-control delectLevel" id="top_ice" name="top_ice" placeholder="Top of Ice">
                                    </div>

                                    <!-- water level -->
                                    <div class="form-group">
                                        <label for="bottom_ice">Bottom of Ice</label>
                                        <input type="text" class="form-control delectLevel" id="bottom_ice" name="bottom_ice" placeholder="Bottom of Ice">
                                    </div>
